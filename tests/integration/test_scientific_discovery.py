...
from src.orchestration.scientific_discovery_engine import ScientificDiscoveryEngine, DiscoveryTask
...
async def test_scientific_discovery_engine_loop():
    ...
    mock_coordinator.coordinate.return_value = CoordinatedResult(
        ...
        synthesis={"conclusion": "AI Empathy is possible", "next_research_question": "How to scale?"}
    )
    
    mock_event_bus = AsyncMock()
    
    # 2. สร้าง Engine
    engine = ScientificDiscoveryEngine(coordinator=mock_coordinator, event_bus=mock_event_bus)
    
    # 3. สร้าง Task
    task = DiscoveryTask(
        task_id="test-task",
        scientific_goal="วิจัย Empathy",
        domain="AI",
        iterations=2
    )
