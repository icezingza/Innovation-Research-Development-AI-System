...
@router.get("")
async def list_swarms(current_user=Depends(get_current_user)):
    ...
    return [
        {
            ...
            "name": t.name,
            "domain": t.domain,
            "description": t.__dict__.get("description", ""),
        }
...
async def activate_swarm(
    ...
    await RequireRole(["admin"])(current_user)
    ...
    result = await db.execute(
        select(TenantSwarm).filter(
            TenantSwarm.tenant_id == tenant_id, TenantSwarm.is_active
        )
    )
    ...
    # Activate new
    new_swarm = TenantSwarm(tenant_id=tenant_id, template_id=template_id)
    db.add(new_swarm)
    await db.commit()
...
async def get_active_swarm(
    tenant_id: str, db=Depends(get_rls_db), current_user=Depends(get_current_user)
):
    result = await db.execute(
        select(TenantSwarm).filter(
            TenantSwarm.tenant_id == tenant_id, TenantSwarm.is_active
