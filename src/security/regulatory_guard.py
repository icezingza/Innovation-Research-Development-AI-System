...
class RegulatoryGuard:
        ...
        for rule_file in sorted(self.rules_path.glob("*.yaml")):
            with open(rule_file, "r", encoding="utf-8") as f:
                ...
                if data and "rules" in data:
                    ...
                    logger.info(
                        f"Loaded {len(data['rules'])} rules from {rule_file.name}"
                    )
        ...
        for rule in self.rules:
            ...
            if "patterns" in rule:
                for pattern in rule["patterns"]:
                    flags = (
                        re.IGNORECASE if not rule.get("case_sensitive", False) else 0
                    )
    ...
    def get_applicable_rules(self, domain: str) -> List[Dict]:
        ...
        return [
            r
            for r in self.rules
            if domain in r.get("domains", []) or "all" in r.get("domains", [])
        ]
