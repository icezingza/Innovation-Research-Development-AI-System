...
class ResilientInferenceRouter:
    ...
    async def generate_with_fallback(
        ...
        **kwargs: Any,
        ...
        try:
            logger.debug(
                f"[{session_id}] กำลังส่งคำสั่งไปยัง Primary Provider ({self.primary.__class__.__name__})"
            )

            # เรียกใช้งาน Cloud API หลัก
            return await self.primary.complete(
                prompt=prompt, system=system_instruction or "", **kwargs
            )

        except Exception as primary_error:
            # ดักจับ Error ทุกรูปแบบ (Timeout, Rate Limit, API Down)
            logger.error(
                f"❌ [{session_id}] Primary Provider ล่มชั่วคราว: {str(primary_error)}"
            )
            ...
            if self.audit_logger and hasattr(self.audit_logger, "log_fallback_event"):
                await self.audit_logger.log_fallback_event(
                    session_id, str(primary_error)
                )

            logger.warning(
                f"🔄 [{session_id}] กำลังเปิดใช้งานโครงข่ายสำรอง: เปลี่ยนเส้นทางไป Local Ollama ค่ะ"
            )
            ...
            try:
                ...
                logger.info(
                    f"✅ [{session_id}] ดึงข้อมูลผ่าน Local Ollama Fallback สำเร็จ ระบบรันต่อได้ไม่หยุดชะงัก"
                )
                ...
                logger.critical(
                    f"🚨 [CRITICAL] [{session_id}] โครงข่ายสำรอง Ollama ล่มเช่นกัน: {str(fallback_error)}"
                )
                raise RuntimeError(
                    f"Inference Pipeline Failed entirely. Primary: {primary_error}, Fallback: {fallback_error}"
                )
