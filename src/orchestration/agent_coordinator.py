...
class AgentCoordinator:
    ...
    async def coordinate(
        ...
        await self._publish(
            TOPIC_COORDINATION_STARTED, {"session_id": s_id, "goal": goal}
        )
        ...
        try:
            ...
            async with asyncio.timeout(self.config.timeout_seconds):
                ...
                sub_questions = [
                    f"Sub-question {i + 1} for {goal}"
                    for i in range(self.config.max_sub_questions)
                ]
            ...
            raise RuntimeError(
                f"Orchestration timed out after {self.config.timeout_seconds}s"
            )
    ...
    async def _run_hypotheses(
        self, sub_questions: list[str], session_id: str
    ) -> list[dict[str, Any]]:
    ...
    async def _run_critiques(
        self, goal: str, hypotheses: list[dict[str, Any]], session_id: str
    ) -> list[dict[str, Any]]:
        ...
        for i, h in enumerate(hypotheses):
            ...
            tasks.append(
                agent.run({"hypothesis": h, "question": goal, "session_id": session_id})
            )

        messages = await asyncio.gather(*tasks)
        return [
            m.content["action"]
            if isinstance(m.content, dict) and "action" in m.content
            else m.content
            for m in messages
            if m and hasattr(m, "content")
        ]
    ...
    async def _publish(self, topic: str, payload: dict[str, Any]) -> None:
        ...
        await self.event_bus.publish(
            RuntimeEvent(topic=topic, source_agent_id="coordinator", payload=payload)
        )
