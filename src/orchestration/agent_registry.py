...
class AgentRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._capabilities: dict[
            str, dict[str, CapabilitySpec]
        ] = {}  # agent_id -> {cap_name: spec}
        self._states: dict[str, AgentState] = {}  # agent_id -> state

    def register(
        self, agent: Any, capabilities: list[CapabilitySpec] | None = None
    ) -> None:
        agent_id = agent.agent_id
        self._agents[agent_id] = agent
        self._states[agent_id] = AgentState()
        self._capabilities[agent_id] = (
            {cap.name: cap for cap in capabilities} if capabilities else {}
        )
    ...
    def find_by_capability(
        ...
        for a_id, caps in self._capabilities.items():
            if (
                ...
                and caps[capability_name].confidence >= min_confidence
            ):
                if a_id in self._agents:
                    candidates.append(
                        (self._agents[a_id], self._states[a_id].current_load)
                    )
