...
class DistributedScalingManager:
    ...
    async def initialize_distributed_cluster(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"🚀 สตาร์ท Worker Pool สำเร็จ รองรับสูงสุด {self.max_parallel_workers} Workers ขนานกัน"
            )
    ...
    async def dispatch_scale_task(self, goal: str, session_id: str) -> Dict[str, Any]:
        ...
        if not self._is_scaled_up:
            raise RuntimeError(
                "กรุณาเรียก initialize_distributed_cluster ก่อนเริ่มกระจายงานค่ะ"
            )
        ...
        task_future = await self.worker_pool.submit(
            self.coordinator.coordinate, goal=goal, session_id=session_id
        )
