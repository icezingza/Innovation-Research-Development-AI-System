...
check("POST /tenants (Tenant A created)", r.status_code in (200, 201, 409),
...
check("POST /tenants (Tenant B created)", r.status_code in (200, 201, 409),
      f"status={r.status_code}")
if r.status_code in (200, 201):
...
check("POST /auth/login (Tenant A)", r.status_code == 200,
      f"status={r.status_code}")
...
if TENANT_B_ID != "existing":
    ...
    check("POST /auth/login (Tenant B)", r.status_code == 200,
          f"status={r.status_code}")
...
r = requests.post(f"{BASE}/research/tasks",
    json={"question": "What is the nature of consciousness?"},
    headers=HEADERS_A,
)
check("POST /research/tasks (Tenant A)", r.status_code in (200, 202),
      f"status={r.status_code}")
...
if TASK_ID:
    ...
    check("GET /trace (admin role, Tenant A)", r.status_code == 200,
          f"status={r.status_code}")
    ...
    if r.status_code == 200:
        ...
        if data.get("events"):
            ev = data["events"][0]
            print(f"     First event: agent={ev.get('agent')}, action={ev.get('action')}")
    else:
        print(f"     Response: {r.text[:200]}")

    # Test: no auth -> 401
    r_noauth = requests.get(f"{BASE}/research/tasks/{TASK_ID}/trace")
    check("GET /trace (no auth -> 401)", r_noauth.status_code == 401,
    ...
    if HEADERS_B:
        ...
        check("GET /trace cross-tenant (Tenant B -> 403/404)", r_b.status_code in (403, 404),
...
check("GET /finops (Tenant A)", r.status_code == 200,
      f"status={r.status_code}")
...
if HEADERS_B:
    ...
    check("GET /finops cross-tenant (Tenant B -> 403)", r_b.status_code == 403,
