...
if os.path.exists("cognition_sim.db"):
    try:
        ...
        print(
            f"  [{GREEN}INFO{RESET}] Cleaned up old cognition_sim.db to ensure reproducible run state."
        )
    except Exception as e:
        print(
            f"  [{YELLOW}WARN{RESET}] Could not remove cognition_sim.db (it might be locked): {e}"
        )

# 1. Start the API Server in the background and pipe output to a log file
PORT = int(BASE.split(":")[-1])
print(
    f"\n{BOLD}[Step 1/7]{RESET} Checking if FastAPI server is already running on port {PORT}..."
)
server_process = None
try:
    ...
    if health_resp.status_code == 200:
        print(
            f"  [{GREEN}INFO{RESET}] Detected existing server running on port {PORT}. Reusing it."
        )
except Exception:
    ...
    sim_env["SQLITE_URL"] = "sqlite+aiosqlite:///cognition_sim.db"
    ...
    print("Waiting 15 seconds for uvicorn to initialize and bind port...")
    time.sleep(15)
...
try:
    ...
    if health_resp.status_code == 200:
        print(
            f"  [{GREEN}PASS{RESET}] Server is healthy and responding to /health endpoint!"
        )
    else:
        print(
            f"  [{RED}FAIL{RESET}] Server health check returned status: {health_resp.status_code}"
        )
        if server_process:
...
try:
    ...
    if reg_resp.status_code in (200, 201):
        ...
        print(
            f"  [{GREEN}PASS{RESET}] Tenant registered successfully. Tenant ID: {tenant_id}"
        )
    elif reg_resp.status_code == 409:
        print(
            f"  [{YELLOW}WARN{RESET}] Tenant already exists in SQLite DB, continuing..."
        )
...
try:
    ...
    if login_resp.status_code == 200:
        ...
        print(
            f"  [{YELLOW}WARN{RESET}] JWT Login rejected, using developer bypass header."
        )
except Exception as e:
...
try:
    ...
    if task_resp.status_code in (200, 202):
        ...
        print(
            f"  [{GREEN}PASS{RESET}] Valid research task accepted! Task ID: {task_id}"
        )
    else:
        print(
            f"  [{RED}FAIL{RESET}] Valid task was rejected: {task_resp.status_code} - {task_resp.text}"
        )
except Exception as e:
...
pii_payload = (
    "นี่คือประวัติของนักวิจัยหลัก รหัสประชากร 1234567890123 ชื่อ สมชาย ดีใจ โปรดทำการวิเคราะห์ยา"
)
print(f'  {YELLOW}Prompt sent:{RESET} "{pii_payload}"')
try:
    task_resp = requests.post(
        f"{BASE}/research/tasks", json={"question": pii_payload}, headers=headers
    )
    if task_resp.status_code == 422:
        err_detail = task_resp.json().get("detail", {})
        print(
            f"  [{GREEN}SUCCESS{RESET}] {BOLD}BLOCKED!{RESET} Server intercepted regulatory violation."
        )
        print(f"    - Violation Type: {err_detail.get('error')}")
        print(
            f"    - Blocked Rule: {err_detail.get('rule_id')} (Thai PDPA ID Card detection)"
        )
        print(f"    - Message: {err_detail.get('message')}")
    else:
        print(
            f"  [{RED}FAIL{RESET}] Leakage attempt NOT blocked! Code: {task_resp.status_code} - {task_resp.text}"
        )
except Exception as e:
...
try:
    task_resp = requests.post(
        f"{BASE}/research/tasks", json={"question": fin_payload}, headers=headers
    )
    if task_resp.status_code == 422:
        err_detail = task_resp.json().get("detail", {})
        print(
        ...
        print(f"    - Violation Type: {err_detail.get('error')}")
        print(
            f"    - Blocked Rule: {err_detail.get('rule_id')} (SEC/BOT Prohibited financial advising)"
        )
        print(f"    - Message: {err_detail.get('message')}")
    else:
        print(
            f"  [{RED}FAIL{RESET}] Financial advice attempt NOT blocked! Code: {task_resp.status_code} - {task_resp.text}"
        )
except Exception as e:
...
try:
    finops_resp = requests.get(f"{BASE}/tenants/{tenant_id}/finops", headers=headers)
    if finops_resp.status_code == 200:
        ...
        print(
            f"    - Budget Forecast: {BOLD}{data.get('budget_depletion_forecast')}{RESET}"
        )
        print(f"    - FinOps Action:  {data.get('recommendation')[:100]}...")
    else:
        print(
            f"  [{RED}FAIL{RESET}] Failed to fetch FinOps metrics: {finops_resp.status_code} - {finops_resp.text}"
        )
except Exception as e:
...
if server_process:
    ...
    server_process.wait()
    server_log.close()
    print(f"  [{GREEN}PASS{RESET}] API Server shut down cleanly. Port {PORT} released.")
else:
    print(
        f"\n{BOLD}[Step 7/7]{RESET} Existing server was used. Leaving it running on port {PORT}."
    )
...
print(
    f"{BOLD}{GREEN}   LIVE SIMULATION SUCCESSFUL! NAMO-FORTRESS IS WORKING AS INTENDED!{RESET}"
)
print(
    f"{BOLD}{GREEN}===================================================================={RESET}\n"
)
